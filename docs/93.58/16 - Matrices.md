...
Sea $\mathbb K = \mathbb{C},\mathbb{R},\mathbb{Q}$, $\mathbb K^{n\times m}=\{A$ es una matriz $n\times m$ con coeficientes en  $\mathbb K\}$
...
- $\exists A^{-1} \Leftrightarrow R(A) = n$
...
- Si $A,B \in \mathbb K^{n\times n}$ y $\exists (A\times B)^{-1} \Rightarrow \exists A^{-1},B^{-1}$
...
1. Si $(Sh)$ es indeterminado, entonces tiene al menos 2 soluciones distintas $\Rightarrow \exists X'\neq0/~~X$ es solucion de $(Sh)$. Como $X=\alpha X'$ es solucion de $(Sh)~~~\forall \alpha\in\mathbb K$, $(Sh)$ tiene infinitas soluciones.
2. Supongamos que $(S)$ es compatible $\Rightarrow \exists X_p \in \mathbb K^m/~~X_p$  es solucion de $(S)$ y todas las soluciones de $(S)$ pueden escribirse como $X = X_p + X_h$. Por eso, $(S)$ tiene tantas soluciones como $(Sh)$ , por lo que si $(S)$ es determinado $\Leftrightarrow (Sh)$ es determinado.
