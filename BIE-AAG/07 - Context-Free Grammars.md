...
We say that a context-free grammar $G = (N,\Sigma,P,S)$ is **ambiguous** if there exists a sentence $w \in L(G)$ that is the result of two or more different parse trees. Otherwise we say that the grammar is **unambiguous**. If a language can be generated by an ambiguous grammar, then it is an **ambiguous language**.
